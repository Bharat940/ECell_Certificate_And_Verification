...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const customUpload = formData.get("mode") === "upload";

    if (!file || !(file instanceof File)) {
      return NextResponse.json(
        {
          error:
            'Missing or invalid file. Send a file in FormData under key "file".',
        },
        { status: 400 },
      );
    }
    ...
    const existingCertNumbers = new Set(
      existing.map((certificate) =>
        certificate.certificateNumber.toUpperCase(),
      ),
    );
