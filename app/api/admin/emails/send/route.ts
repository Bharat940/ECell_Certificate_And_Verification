...
import Event, { IEvent } from "@/models/Event"; // Required for model registration/population
import {
  renderEmailTemplate,
  ...
  try {
    ...
    for (const cert of certificates) {
      ...
      try {
        ...
        logger.info(
          "EMAIL",
          `Sent certificate email to ${cert.participantEmail}`,
        );
      } catch (error: unknown) {
        failureCount++;
        const errorMessage =
        ...
        logger.error(
          "EMAIL",
          `Failed to send email to cert ${certIdStr}`,
          errorMessage,
        );
      }
    }
