...
// Extracts a year-range like "2022-2026" or "2022 – 2026" from a string
const extractTenure = (title: string): string | null => {
  const match = title.match(/(\d{4})\s*[-–—]\s*(\d{4})/);
  if (match) return `${match[1]} – ${match[2]}`;
  return null;
};

// Parses "Ishaan Mathur | Team Supervisor (Lead)" → { name, position }
// If no pipe, returns full string as name and null position
const parseNameAndPosition = (
  raw: string,
): { name: string; position: string | null } => {
  const pipeIndex = raw.indexOf("|");
  if (pipeIndex === -1) return { name: raw.trim(), position: null };
  return {
    name: raw.slice(0, pipeIndex).trim(),
    position: raw.slice(pipeIndex + 1).trim() || null,
  };
};
...
const getCertificateDetails = (
  ...
  const { name, position } = parseNameAndPosition(participantName);

  const isCommitteeType =
    eventLower.includes("tenure") ||
    eventLower.includes("committee") ||
    eventLower.includes("membership") ||
    eventLower.includes("contribution") ||
    eventLower.includes("exit") ||
    eventLower.includes("service") ||
    eventLower.includes("fellowship") ||
    eventLower.includes("alumni");

  const isRecognitionType =
    !isCommitteeType &&
    (eventLower.includes("graduation") ||
      eventLower.includes("passout") ||
      eventLower.includes("recognition") ||
      eventLower.includes("honor"));

  if (isCommitteeType) {
    return {
      type: "committee",
      name,
      position: position ?? "Core Committee Member",
      tenure: extractTenure(eventTitle),
      team: organizer || "E-Cell RGPV",
    };
  }
...
export default function VerifyPage({ params }: VerifyPageProps) {
  ...
  return (
    <main className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      <div className="absolute inset-0 bg-[radial-gradient(#334155_1px,transparent_1px)] bg-size-[16px_16px] opacity-20"></div>
      ...
      <div className="w-full max-w-3xl relative z-10 mx-auto">
        <div className="flex flex-col items-center mb-8">
          ...
          <div className="text-center">
            ...
            <div className="inline-flex items-center gap-2 bg-slate-900/80 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-slate-800 flex-wrap justify-center shadow-sm">
              ...
              <span className="font-mono text-white font-semibold tracking-wider text-sm sm:text-base">
                {certificateNumber}
              </span>
              {(!data || data.valid || isLoading) && (
                <button
                  onClick={() =>
                    copyToClipboard(certificateNumber, "Certificate number")
                  }
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.5)] border border-slate-800 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-blue-600 via-cyan-500 to-blue-600"></div>
          {isLoading ? (
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-6 text-center">
                ...
                {(() => {
                  const details = getCertificateDetails(
                    ...
                    data.certificate?.event.title || "",
                    data.certificate?.event.organizer || "",
                  );

                  if (details.type === "committee") {
                    return (
                      <div className="space-y-6">
                        {/* Hero Name Block */}
                        <div className="relative rounded-2xl overflow-hidden">
                          <div className="absolute inset-0 bg-linear-to-br from-blue-900/40 via-slate-800/60 to-cyan-900/30 backdrop-blur-sm"></div>
                          <div className="absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-blue-400/60 to-transparent"></div>
                          <div className="relative p-6 sm:p-8 text-center">
                            <h3 className="text-3xl sm:text-4xl font-bold text-white tracking-tight mb-4">
                              {details.name}
                            </h3>
                            {/* Position Badge */}
                            <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/30 text-blue-300 font-semibold px-4 py-1.5 rounded-full text-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <div
                            className={`bg-cyan-500/5 border border-cyan-500/20 rounded-xl p-4 text-center ${!details.tenure ? "sm:col-span-2" : ""}`}
                          >
                            <div className="flex items-center justify-center gap-1.5 mb-1">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        a.href = blobUrl;
                        const safeName =
                          data.certificate.participantName.replace(
                            /[^a-z0-9]/gi,
                            "_",
                          );
                        ...
                        const attachUrl =
                          data.certificate.certificateUrl.replace(
                            "/upload/",
              ...
              {data.certificate?.certificateUrl && (
                <div className="bg-slate-950/50 border border-slate-800/80 rounded-xl p-3 sm:p-4 mt-8">
                  ...
                  <div className="bg-slate-900 rounded-lg overflow-hidden relative w-full pt-[70%] sm:pt-0 sm:h-150 border border-slate-800">
                    <iframe
                ...
                <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-5 mt-8 text-left">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-400 text-sm leading-relaxed">
                      Please verify the certificate number carefully. If you
                      believe this is an error and you successfully participated
                      in the event, please contact the E-Cell RGPV team.
                    </p>
                  </div>
                </div>
              </div>
