  ...
  useEffect(() => {
    if (isOpen) {
      fetch("/api/admin/templates")
        ...
        .then((data) => {
          if (data.success) {
            const activeBlueprints = data.templates.filter(
              (bp: Blueprint) =>
                !bp.isArchived || bp._id === formData.templateId,
            );
      ...
      <div className="bg-slate-900 rounded-2xl p-6 max-w-md w-full border border-slate-800">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              value={formData.templateId || formData.template}
              onChange={(e) => {
                const val = e.target.value;
                if (val.endsWith(".html")) {
                  onFormChange.setTemplate(val);
                  onFormChange.setTemplateId("");
                } else {
                  onFormChange.setTemplate("");
                  onFormChange.setTemplateId(val);
                }
              }}
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500"
            >
              <optgroup
                label="Database Blueprints (Recommended)"
              ...
              >
                ...
                {blueprints.length === 0 && (
                  <option disabled>No database templates found</option>
                )}
              </optgroup>

              <optgroup
                label="Legacy File Templates"
            ...
            <p className="text-xs text-slate-400 mt-1">
              Choose a modern blueprint or a legacy file design
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex-1 bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white py-2 rounded-lg transition-all disabled:opacity-50 cursor-pointer"
            >
              {isEditMode
                ? isLoading
                  ? "Updating..."
